...
> **Upgrading:** One breaking change, for scripts that read grove's JSON: the session field was renamed from `tmux` to `session`. `grove ls --json` now reports `.worktrees[].session` (was `.worktrees[].tmux`), and `grove here --json` reports `.session.{session,status}` (was `.tmux.{session,status}`). Values are unchanged under tmux; under herdr they read `active` / `open`. Otherwise drop-in: tmux users see no behavior change, and shell integration is unchanged, so no re-source is needed. Highlights: **[herdr](https://herdr.dev) support** (`[mux] backend = "herdr"`), with each worktree's coding-agent state in `grove ls` and the dashboard, and a herdr plugin (`herdr plugin install lost-in-the/grove/integrations/herdr --ref v0.11.0`); **`[session] open_in`**; `grove doctor`'s closing action list; and opt-in per-slot git metadata for agent stacks.
...
- A **herdr plugin** in [`integrations/herdr/`](integrations/herdr/README.md): a workspace status action, plus `worktree.created` and `worktree.opened` hooks that point at `grove adopt` when a worktree appears through herdr's UI rather than grove's. Install with `herdr plugin install lost-in-the/grove/integrations/herdr --ref v0.11.0`. There is deliberately no dashboard pane — herdr's sidebar already lists every worktree.
- The plugin keeps an **untracked marker in herdr's sidebar**: a `grove=untracked` workspace token on every worktree grove does not track, cleared as soon as grove does, and re-reported by a startup hook after herdr restarts (herdr drops tokens). It shows wherever your `[ui.sidebar.spaces]` rows name `$grove` — see the plugin README for the row to add.
- **`grove adopt`** brings an adopted worktree's herdr workspace in line: grove's canonical `{project}-{name}` label instead of the branch name herdr gave it, a named tab, and no untracked marker.
- **herdr named sessions.** Each herdr session is an independent server, and herdr itself happily opens the same checkout in two of them. Grove now looks across the running sessions when the one it is talking to has no workspace for a worktree: outside herdr it adopts the existing workspace — focusing it there and attaching with `herdr --session NAME` — and inside a pane of another session it says where the worktree is open and just changes directory, rather than opening a duplicate. `grove rm` closes the worktree's workspace in every running session, so no orphan is left pointing at the deleted directory. Calls to other sessions run under a 250ms budget and in parallel, so a wedged session cannot stall a command (it is skipped, and `grove rm` warns that its copy may remain).
- **Renamed worktrees under herdr.** herdr's record of a workspace's path never follows `grove rename`, so a new worktree created at a renamed one's old path used to take over — and re-label — the renamed worktree's workspace, and `grove rm` of the new one closed it. Grove now recognizes a workspace whose shells sit in another worktree of the repository and leaves it alone, declining with a hint instead.
...
- `grove doctor` reports herdr availability, and — when herdr is the active backend — whether its server is running and speaks the installed CLI's protocol. After a herdr upgrade the old server refuses every call until it is restarted; doctor says so instead of reporting it reachable, and `grove to` falls back to a plain directory change with a one-line warning rather than failing.
...
Verified end to end against herdr 0.8.0 and 0.9.1: create, list, switch, rename, remove, `grove open` with a session command, directory drift correction, agent state, named sessions, and the plugin's hooks all round-trip correctly, and `grove ls` stays around 50ms with the herdr backend active. CI runs the same validation suite (`scripts/validate-herdr.sh`) against a pinned headless herdr.
...
- Under herdr, `grove open` with `[session] popup = true` falls back to a full-window workspace switch — herdr has no popup placement grove can use.
- Under herdr with shell integration, grove focuses the target workspace before printing its `Run: herdr` hint, so running the hint lands there instead of on whatever workspace herdr last had focused.
- The `grove ls` session column is **named after the active backend** (`TMUX`, `HERDR`, or `SESSION`) instead of always reading `TMUX`. Under herdr its values are `active` / `open` rather than `attached` / `detached`: herdr's focus is a single server-wide "current workspace" that stays set with no client attached, so it cannot say whether anyone is looking. The same values appear in the `session` field of `grove ls --json` / `grove here --json` (renamed from `tmux` — see the upgrade note) and on the dashboard's badges.
- **Grove never calls `herdr workspace create`**, nor `worktree create` / `worktree remove`. Every session goes through `herdr worktree open`, which adopts an existing checkout; for the repository's own checkout herdr answers by opening its parent workspace — the one its sidebar groups the project's worktrees under — so `grove to root` lands there.
...
- **`[plugins.docker.external.agent] export_git_metadata`** opts an agent stack slot into `GROVE_SLOT_GIT_SHA` / `GROVE_SLOT_GIT_BRANCH` exports, resolved host-side from the slot's worktree path — the consumer can't compute this itself, since mise evaluates per-directory rather than per-slot and a linked worktree's `.git` file points outside the container's mount. Off by default (the two git lookups run on every agent-stack compose command for the slot — up, down, run, exec — not just start, since compose re-interpolates the template each time); a resolution failure warns and degrades to empty rather than failing stack start.
- `grove here --check-mount --require-current` exits with a distinct code (`13`, `MountCheckMismatch`) when the current worktree isn't the one the stack's env file is configured for — a pre-test guard for "am I even in the worktree this stack serves?", separate from the env-vs-container `MountDrift` (`12`) check. It fails closed the same way when the configured worktree can't be resolved at all (env var unset, or the worktree manager errored) — a guard that can't tell which worktree the stack serves must not silently pass.
...
- `grove repair` no longer flags a sibling project's own root session (e.g. a "grove-web" session, matching prefix "grove-") as an orphan of this project when the trimmed remainder is the reserved name "root" — a real worktree of this project can never produce that session name (#116).
- Agent-stack template failures name the actual problem instead of an opaque compose error. `docs/CONFIGURATION_REFERENCE.md` now documents exactly which variables an agent compose template may interpolate (`<env_var>`; `AGENT_SLOT` when a slot is in use; and, opt-in, the `GROVE_SLOT_GIT_*` pair), and when `grove up --isolated` fails after compose warns a template variable is unset, the error now names the unset variable(s) plus the variable names grove exported, rather than leaving the user to trace `invalid spec: :/app:delegated: empty section between colons` back to a template typo.
- **Agent slot occupancy is now scoped to the shared compose stack, not the calling grove project (#147).** When `[plugins.docker.external] path` is mounted by more than one grove project (e.g. the stack repo itself and an app repo nested inside it), each project used to track slots 1..max_slots purely from its own `.slots.json` and had no way to see the other's allocations — `grove up` could hand out a slot another project's stack already occupied, surfacing as an opaque `container name already in use` daemon error (or silent double-occupancy when the template doesn't pin `container_name`), and `grove down --slot N` couldn't clean it up. Grove now checks running containers at the resolved stack path (via the `com.docker.compose.project.working_dir` label) before allocating a slot and skips ones a different compose project already holds; if a race still produces a container-name conflict on `docker compose up`, the error now names the compose project holding the slot instead of passing through the raw daemon error. `grove ps` does the same reconciliation and reports a foreign-held slot as `held by <compose-project>` rather than attributing it to a worktree in the current project.
...
- The dashboard no longer spawns git to re-discover `.grove` on every refresh: TUI config loads anchor at the already-resolved project root instead of walking up from the cwd (#142) — which also guarantees the dashboard reads the project it was launched for, not whatever project the cwd happens to be in.
