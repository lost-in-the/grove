...
> [!NOTE]
> **Beta — pre-1.0 active development.** Commands, config schema, and behavior may change between `0.X.0` releases. See [CHANGELOG.md](CHANGELOG.md) for migration notes; please [report issues](https://github.com/lost-in-the/grove/issues) and share feedback.
...
```bash
go install github.com/lost-in-the/grove/cmd/grove@main
```

Edge builds may include breaking changes that haven't been documented yet. Pin to a tagged release for stability.

### Updating

| Install method | Update command |
|----------------|----------------|
| Homebrew | `brew update && brew upgrade lost-in-the/tap/grove` |
| `go install` | Re-run `go install github.com/lost-in-the/grove/cmd/grove@latest` |
| Edge (`@main`) | Re-run `go install github.com/lost-in-the/grove/cmd/grove@main` |
